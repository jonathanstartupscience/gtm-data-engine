 ...
 *
 * Last reviewed: 2026-06-23 (Reply notifications & speed-to-lead: every reply posts a Google Chat alert that round-robins to a sales rep and deep-links to the Inbox where the rep claims it and replies in-app through Bison. HubSpot promotion on reply (MQL / lead status REPLIED) is built but held back until the CRM objects are finalized — enable with REPLY_HUBSPOT_SYNC=1. Plus: workspace-centric Email Engine — one Bison account, a workspace per persona with its own API key; a workspace selector scopes Campaigns/Sequences/Experiments/Inbox/Performance, and pushes auto-filter to the workspace persona).
 */
...
export const CONCEPTS: KbSection[] = [
  ...
  { heading: 'Email Engine workspaces', body: 'The Email Engine mirrors Email Bison’s structure: one Bison account with a separate WORKSPACE per persona — ESOs, Founders, Investors, Providers, Advisors (plus an inactive Community Funding). Each workspace is an isolated tenant in Bison with its OWN API key, its own sender inboxes, campaigns, sequences, and inbox. Pick the active workspace from the selector at the top of the Email Engine nav; everything you then see — Campaigns, Sequences, Experiments, Inbox, Performance — is scoped to that one workspace. Each workspace is bound to a persona, so when you push contacts, the canonical store is automatically filtered to that workspace’s persona (you can narrow further by sub-type). Set each workspace’s Bison API key under Settings (“Email Bison key — <workspace>”); a workspace with no key of its own falls back to the global Email Bison key. Switching workspace is how you move between, say, the Founders book of business and the Investors one — they never mix contacts, campaigns, or replies.' },
...
export const PAGES: Record<string, KbPage> = {
  ...
  '/experiments': {
    ...
    intro: 'Run multiple sequences head-to-head against one audience in the selected workspace, split evenly or by weight, and prune losers / scale winners without reshuffling anyone already in flight.',
    sections: [
      { heading: 'Scoped to the workspace', body: 'An experiment belongs to the active workspace and draws from that workspace’s persona segment. This is exactly the “prepare a big batch for a persona, test several messages, then add more and iterate” loop: the experiment owns the batch — push to allocate the current eligible contacts across arms, then later add more contacts for the persona and push again to flow only the new ones.' },
    ...
    steps: [
      ...
      'Experiments → New experiment: name it, pick the audience (persona/sub-type), add an arm per campaign with a weight.',
