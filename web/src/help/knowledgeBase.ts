...
export const PAGES: Record<string, KbPage> = {
  ...
  '/email/workspaces': {
    ...
    sections: [
      ...
      { heading: 'Bison instance URL (one shared setting)', body: 'Because the whole account lives on one Bison instance, the instance URL is a single shared setting, not per workspace — set it once on Settings (“Email Bison instance URL”, e.g. https://send.yourdomain.com/api). If sync or the inbox fails with a 500 / “check the key/instance URL”, the base is usually the culprit: the built-in default (dedi.emailbison.com) won’t match a custom/dedicated instance.' },
