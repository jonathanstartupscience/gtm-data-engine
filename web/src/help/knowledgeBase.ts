...
export const PAGES: Record<string, KbPage> = {
  ...
  '/discover-contacts': {
    route: '/discover-contacts', workspace: 'data', title: 'Discover Contacts',
    intro: 'Find NET-NEW people across every company, not just your account list. Search Airscale by job title and keyword, and each match (plus their company) lands in your store. Use this when you need people at companies you don’t have yet; use Find Contacts when you already have the companies.',
    sections: [
      { heading: 'Step 1 — who to find', body: 'Search by job titles to include, a keyword (matches title/bio/skills/education — e.g. “university”), and location. A title preset pre-fills common roles; edit freely. Set a max so you bound how many people are sourced, and optionally a persona tag to label them.' },
      { heading: 'Step 2 — discover (cheap)', body: 'A scope preview shows how many people match and the estimated Airscale listing cost. Running adds each person to your store with name, title, and LinkedIn, and creates their company if it’s new (left untyped for the classifier). No emails yet.' },
      { heading: 'Step 3 — find emails (separate)', body: 'Finding emails is slower and costs about ten times as much per person, so it’s a second, separately-confirmed step that only runs on discovered contacts still missing an email. Review who you got first, then find emails. Both steps re-check the live cost before spending.' },
      { heading: 'Why two steps', body: 'It mirrors the app’s credit-safety pattern: list cheaply, review, then spend on the expensive part only for the people you keep. Verify the found emails afterward on the Contacts tab.' },
    ],
    steps: [
      'Add job titles and/or a keyword, a location, and a max.',
      'Review the scope & cost, then Discover people.',
      'Review the discovered contacts.',
      'Run Find emails on those still missing one, then verify.',
    ],
  },
  '/import': {
