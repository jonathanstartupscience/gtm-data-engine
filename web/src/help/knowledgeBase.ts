 ...
 *
 * Last reviewed: 2026-06-24 (Workspaces page: each persona workspace’s Email Bison API key and reply-routing override now live on a dedicated Workspaces page in the Email Engine nav — not scattered on global Settings. Settings keeps the global/fallback Bison key plus HeyReach/Anthropic/Google Chat; the six per-workspace Bison rows moved off it. Reply routing is now a shared component reused on both pages. Earlier — Reply notifications & speed-to-lead: every reply posts a Google Chat alert that round-robins to a sales rep and deep-links to the Inbox where the rep claims it and replies in-app through Bison. HubSpot promotion on reply (MQL / lead status REPLIED) is built but held back until the CRM objects are finalized — enable with REPLY_HUBSPOT_SYNC=1. Plus: workspace-centric Email Engine — one Bison account, a workspace per persona with its own API key; a workspace selector scopes Campaigns/Sequences/Experiments/Inbox/Performance, and pushes auto-filter to the workspace persona. Experiments doc rewritten in plain terms (horse-race framing, why pinning protects the A/B comparison, the preview→push→iterate loop, and the “link sequence” dropdown is provenance-only — emails send from the arm’s campaign). Community Funding is now an active workspace (all six match Bison names exactly), and a campaigns status light (green = an active campaign is sending, red = none) sits under the workspace selector).
 */
...
export const PAGES: Record<string, KbPage> = {
  ...
  '/email/workspaces': {
    route: '/email/workspaces', workspace: 'email', title: 'Workspaces',
    intro: 'One place to configure each persona workspace: its Email Bison API key and its reply-routing override. Set these once and every campaign, sequence and experiment in that workspace is attributed to the right persona.',
    sections: [
      { heading: 'One key per workspace', body: 'The Email Engine mirrors Email Bison: one account, a separate workspace per persona (ESOs, Founders, Investors, Providers, Advisors, Community Funding), each with its OWN API key. Paste each workspace’s key here — get it from Email Bison by switching into that workspace → API. A workspace with no key of its own falls back to the global Email Bison key (set on Settings), but for clean per-persona attribution give each its own.' },
      { heading: 'Reading each card', body: 'Each workspace shows whether it has its own key (with a masked preview once saved) or is using the global fallback, plus a sending light (green = at least one active campaign in Bison, idle = none). Save replaces the key; Remove deletes the workspace’s own key so it falls back to global again.' },
      { heading: 'Reply routing per workspace', body: 'Each card also sets that workspace’s reply-routing override — the round-robin roster of reps that reply alerts cycle through and the Google Chat space they post to. Because routing follows the active workspace, select a workspace (its row highlights) to edit its routing; the global default lives on Settings → Reply routing.' },
      { heading: 'Where the other keys live', body: 'Only the per-workspace Bison keys are here. The global/fallback Bison key, HeyReach, Anthropic, and the Google Chat webhook are on Settings. The master encryption key that protects all of them, APP_ENCRYPTION_KEY, is set once in Railway — it can’t live in the app because it’s what secures the app’s own stored keys.' },
    ],
    steps: [
      'Open Email Bison and switch into a persona workspace → API → copy its key.',
      'On this page, paste that key into the matching workspace card and Save.',
      'Repeat for all six workspaces (an unset one falls back to the global Bison key).',
      'Optionally select a workspace and set its reply-routing roster + Google Chat override.',
      'Confirm each card shows “own key” — then build campaigns, sequences and experiments per workspace.',
    ],
  },
  '/campaigns': {
    ...
    sections: [
      { heading: 'Scoped to the active workspace', body: 'You only see (and build) campaigns for the workspace selected at the top of the Email Engine nav. Sync pulls from that workspace’s Bison (its own API key); new campaigns are created there. Switch workspace to work on a different persona’s book.' },
      { heading: 'Build vs Sync', body: 'Build creates a campaign + sequence + schedule in this workspace’s Bison from here. Sync mirrors campaigns you built in Bison so you can monitor them.' },
  ...
  '/experiments': {
    ...
    steps: [
      'Build one campaign per message you want to test (Campaigns → New campaign, attach the sequence). Each campaign carries exactly one sequence.',
      'Experiments → New experiment: name it, pick the audience (persona/sub-type), and add one arm per campaign. Start with equal weights for an even split.',
      'Create, then open the experiment and read the preview (segment size + new contacts per arm).',
      'Distribute & push (confirm-gated): contacts are split, pinned, and sent to each arm’s campaign.',
  ...
  '/inbox': {
    ...
    sections: [
      ...
      { heading: 'How replies arrive', body: 'In real time via the Email Bison webhook (once BISON_WEBHOOK_SECRET is configured), or on demand with “Sync replies”. Every new reply also posts a Google Chat alert that round-robins to the next sales rep and deep-links straight back here. (Automatic HubSpot promotion on reply is built but currently held back until the CRM objects are finalized.) The nav badge counts unread positive replies so you never miss a warm one.' },
  ...
  '/settings': {
    ...
    sections: [
      ...
      { heading: 'Google Chat for reply alerts', body: 'Add the “Google Chat webhook (reply alerts)” key — create an incoming webhook in the Chat space you want alerts to land in (Space → Apps & integrations → Webhooks). Membership of that space controls who gets alerted.' },
      { heading: 'Reply routing (round-robin)', body: 'Under “Reply routing”, list the sales reps that reply alerts should cycle through. Set a global default, or override it for the active Email-Engine workspace, and optionally point a scope at a different Google Chat space. Each new reply tags the next rep in turn.' },
    ],
  },
};
