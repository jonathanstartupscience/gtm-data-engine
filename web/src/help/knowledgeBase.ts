...
export const CONCEPTS: KbSection[] = [
  ...
  { heading: 'Connecting tools (Settings)', body: 'There is ONE global Settings panel for the whole GTM system — reached from the ⚙ Settings link pinned at the bottom of the sidebar, the same in every workspace (Data, Email, LinkedIn). It holds everything account-wide: vendor API keys (encrypted, no redeploy — or set them as Railway env vars), the read-only Connectors status view, and live credit balances for metered vendors. The Anthropic key powers everything AI: the in-app classifier (fast/cheap Claude Haiku), the AI cold-email sequence writer (Claude Opus, for copy quality), and reply triage (Opus — reads each inbound reply, suggests a strategy, extracts referral contacts). A default Google Chat incoming-webhook URL is also set here — where reply alerts post unless a workspace overrides it (see “Reply notifications & speed-to-lead”). Per-workspace Email settings (each persona’s Bison key, persona scope, reply roster) are NOT global — they live on Email Engine → Workspaces.' },
...
export const PAGES: Record<string, KbPage> = {
  ...
  '/settings': {
    ...
    sections: [
      { heading: 'What’s global (here)', body: 'Vendor API keys (HubSpot, HeyReach, Anthropic, the default Google Chat reply-alert space, the Email Bison instance URL), the read-only Connectors status view, and live credit balances for the metered vendors. These are account-wide and apply across all three engines.' },
      { heading: 'How keys resolve', body: 'A key set here is used first; otherwise the Railway env var is used. So existing keys keep working and anything set here overrides them. Storing keys in-app requires APP_ENCRYPTION_KEY set once in Railway — until then the fields are disabled and keys come from env vars.' },
      { heading: 'Anthropic key', body: 'Powers everything AI in the system: the company classifier (Classify), the AI cold-email sequence writer, and reply triage (the agent that reads each inbound reply, suggests a response strategy, and extracts referral contacts). One key, set here, enables all three.' },
      { heading: 'Google Chat for reply alerts', body: 'The “Google Chat webhook” key here is the DEFAULT alert space — reply alerts post here unless an Email-Engine workspace overrides it with its own space. Create an incoming webhook in the Chat space you want (Space → Apps & integrations → Webhooks); membership of that space controls who gets alerted.' },
      { heading: 'What’s NOT here (per-workspace)', body: 'Each Email-Engine persona workspace has its OWN Bison API key, persona scope, and reply-routing roster (the round-robin rep list + optional own Chat space). Those are per-workspace, so they live on Email Engine → Workspaces — switch workspace in the nav first. Settings links over to them.' },
    ],
  },
};
