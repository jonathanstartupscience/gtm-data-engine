...
export const PAGES: Record<string, KbPage> = {
  ...
  '/email/workspaces': {
    ...
    intro: 'Settings for the workspace you’re currently in: its Email Bison API key, its Bison instance URL, and its reply-routing roster. Like the rest of the Email Engine, this page is scoped to the workspace picked in the nav selector — switch workspace there to configure another persona.',
    sections: [
      { heading: 'One key per workspace — no global key', body: 'The Email Engine mirrors Email Bison: one account, a separate workspace per persona (ESOs, Founders, Investors, Providers, Advisors, Community Funding), each authenticating with its OWN API key. There is no account-wide / global Bison key — Bison has no shared sending identity. Get this workspace’s key from Email Bison (switch into the workspace → API), paste it here, Save. A workspace with no key simply can’t send until one is set.' },
      { heading: 'Bison instance URL', body: 'The API base for your Bison instance, e.g. https://send.yourdomain.com/api. If sync or the inbox fails with a 500 / “check the key/instance URL”, the base is usually the culprit — the app’s built-in default (dedi.emailbison.com) won’t match a custom/dedicated instance. Set it per workspace here, or set one shared default for all workspaces on Settings (“Email Bison instance URL”). Leave the per-workspace field blank to use that shared default.' },
