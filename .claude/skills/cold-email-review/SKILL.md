...
- **D1 Whitelist** — tags are single-brace UPPERCASE; only approved ones appear: `{FIRST_NAME}`, `{COMPANY}`, `{TITLE}`, `{LAST_NAME}`, `{TRIGGER}`, `{MAGNET_LINK}`, `{SENDER_LINKEDIN}`. Any single-brace tag not in the list, and ANY old-style `{{double_brace}}` tag (wrong dialect — renders literally), is a defect. Quote it.
